...
const AdminPage = ({ user, client }) => {
  ...
  if (data) {
    ...
    const showFilter = user && <div>
      ...
      <input className="all-filter" type="radio" name="filter" defaultChecked onChange={() => handleAllSubmissions()} />
      <label>View Favorites</label>
      <input className="fav-filter" type="radio" name="filter" onChange={() => handleFavorites()}  />
    </div>
